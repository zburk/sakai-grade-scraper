...
def main():
    ...
    all_sites = sak.session.executeRequest(
        type="GET", url="/site.json", params={"_limit": 100}
    )
    ...
    while True:
        ...
        for site in fall_sites:
            gradebook = sak.session.executeRequest(
                type="GET", url="/gradebook/site/" + site["id"] + ".json"
            )

            if site["id"] in previous_assignments and previous_assignments[
                site["id"]
            ] != len(gradebook["assignments"]):
                gmail_user = str(os.getenv("GMAIL_USERNAME"))
                gmail_password = str(os.getenv("GMAIL_PASSWORD"))

                sent_from = gmail_user
                to = [str(os.getenv("GMAIL_USERNAME"))]
                subject = "Grade Posted"
                body = (
                    "<pre>"
                    + json.dumps(gradebook["assignments"], indent=4, sort_keys=True)
                    + "</pre>"
                )
